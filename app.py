...
THEORY_DATA = {
    "Cộng Trừ phạm vi 1000": {"khai_niem": "Thực hiện phép tính cộng, trừ với các số có 3 chữ số.", "phuong_phap": "Bước 1: Đặt tính thẳng cột (Hàng trăm thẳng hàng trăm, chục thẳng chục, đơn vị thẳng đơn vị).\nBước 2: Tính từ PHẢI sang TRÁI (từ hàng đơn vị trước).\nLưu ý: Nhớ cộng/trừ số nhớ sang hàng tiếp theo!", "sai_lam": "Quên không cộng số nhớ, hoặc đặt tính các số không thẳng cột với nhau."},
    "Bảng Nhân Chia (2,3,4,5)": {"khai_niem": "Nhân là cộng nhiều số giống nhau. Chia là chia đều thành các phần bằng nhau.", "phuong_phap": "- Học thuộc lòng các bảng nhân, chia từ 2 đến 5.\n- Mẹo: Phép nhân có tính chất giao hoán (Ví dụ: 3 x 4 cũng bằng 4 x 3 = 12).", "sai_lam": "Nhầm lẫn giữa dấu cộng và dấu nhân (Ví dụ sai: 2 x 3 = 5)."},
    "Đại lượng & Thời gian": {"khai_niem": "Đo độ dài (m, dm, cm, mm), cân nặng (kg) và thời gian (ngày, giờ, tháng).", "phuong_phap": "- Ghi nhớ quy đổi: 1m = 10dm = 100cm; 1dm = 10cm.\n- Muốn cộng trừ các số đo, chúng phải CÙNG MỘT ĐƠN VỊ đo.", "sai_lam": "Cộng trực tiếp các số khác đơn vị (Ví dụ sai: 1m + 20cm = 21). Phải đổi 1m = 100cm rồi mới cộng: 100 + 20 = 120cm."},
    "Hình học (Chu vi)": {"khai_niem": "Chu vi của một hình là độ dài đường bao quanh hình đó.", "phuong_phap": "- Chu vi hình tam giác = Tổng độ dài 3 cạnh.\n- Chu vi tứ giác/chữ nhật = Tổng độ dài 4 cạnh.\n(Nhớ kiểm tra xem các cạnh đã cùng đơn vị đo chưa nhé).", "sai_lam": "Chỉ cộng 2 cạnh rồi dừng lại, hoặc quên ghi đơn vị ở đáp án."},
    "Toán Tư Duy (Nền tảng NTT)": {"khai_niem": "Các bài toán đố đòi hỏi con phải suy luận, vẽ sơ đồ hoặc tưởng tượng tình huống thực tế.", "phuong_phap": "Bước 1: Đọc thật kỹ đề, gạch chân các con số.\nBước 2: Vẽ sơ đồ tóm tắt hoặc nháp ra giấy.\nBước 3: Tìm phép tính ẩn giấu. (Ví dụ toán tính tuổi: Mỗi năm mỗi người đều tăng thêm 1 tuổi).", "sai_lam": "Đọc lướt đề bài, thấy số là cộng/trừ ngay mà không suy nghĩ ý nghĩa của bài toán."}
}
        ...
        if total_questions > 0:
            ...
            if index < total_questions:
                ...
                with st.form(key=f"form_{q['id']}"):
                    st.write(f"**Đề bài ({q['level']}):** {q['question']}")
...
# ==========================================
    ...
    if 'exam_generated' not in st.session_state or not st.session_state.exam_generated:
        ...
        exam_qs = random.sample(q_m1, min(5, len(q_m1))) + random.sample(q_m2, min(7, len(q_m2))) + random.sample(q_m3, min(3, len(q_m3)))
